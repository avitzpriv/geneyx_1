...
const startMultipartUpload = (_task, _filePath) => {
  ...
  const inStream = fs.createReadStream(_filePath, { highWaterMark: PART_SIZE })
  const fileStat = fs.statSync(_filePath)
  const fileSize = fileStat.size   // In bytes
  ...
  const env = {
    ...
    numPartsLeft: Math.ceil(fileSize / PART_SIZE),  // How many parts are left
  }
...
const uploadPartsRunner = async (multipart, env) => {
  ...
  for (var rangeStart = env.partNum ;rangeStart < env.bufferSize; rangeStart += PART_SIZE) {
    console.log('In FOR loop, rangeStart = ', rangeStart, ', buff len: ', env.bufferSize)
    ...
    const end = Math.min(rangeStart + PART_SIZE, env.bufferSize)
    const partParams = {
            Body: env.buffer,
            Bucket: process.env.S3_BUCKET,
            Key: env.fileKey,
